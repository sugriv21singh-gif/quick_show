...
1. **Clone the repository**
   ```console
   git clone https://github.com/sugriv21singh-gif/quickshows.git
   cd quickshows
   ```
...
4. **Environment Configuration**
   ...
   **Client (.env)**
   ...
   # Alternatively this repo accepts either of these names:
...
1. **Initialize Git and push to GitHub**
   ...
   git init
   git add .
   git commit -m "Initial QuickShow project setup with Clerk fix"
   git branch -M main
   git remote add origin https://github.com/sugriv21singh-gif/quickshows.git
   git push -u origin main
   ```

   If you do not yet have a GitHub repository, create one at https://github.com/new and replace the remote URL above with your new repo.
...
1. Create a Vercel account and import the repository: `https://github.com/sugriv21singh-gif/quickshows`
...
4. Deploy the app from Vercel.

### Clerk redirect URLs

In Clerk Dashboard, add these redirect origins/path settings:
- `http://localhost:5173`
- `https://<your-vercel-project>.vercel.app`
- `https://<your-vercel-project>.vercel.app/admin`

### Verify login flow after deploy

1. Open the deployed site URL from Vercel.
2. Go to `/login` and sign in using your Clerk account.
3. If the login page does not load or returns a Clerk error, confirm:
   - `VITE_CLERK_PUBLISHABLE_KEY` / `NEXT_PUBLIC_CLERK_PUBLISHABLE_KEY` is set in Vercel.
   - `CLERK_SECRET_KEY` is set in server environment.
   - The same publishable key is used in client and server config.
   - Allowed redirect URLs in Clerk include your deployed domain.
4. If login succeeds, verify user booking flow:
   - Browse movies
   - Select seats
   - Create a booking
   - Complete payment or mock payment flow

> Use the same Clerk publishable key in both client and server environments to avoid login errors and ensure authentication works correctly.
